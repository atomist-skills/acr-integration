...
exports.closePullRequests = exports.persistChanges = void 0;
const fs = require("fs-extra");
const git = require("../git/operation");
 ...
 */
async function persistChanges(ctx, project, strategy, push, pullRequest, commit) {
    const gitStatus = await git.status(project);
    if (gitStatus.isClean && (!(commit === null || commit === void 0 ? void 0 : commit.editors) || commit.editors.length < 1)) {
        return status.success(`No changes to push`);
    }
    const createPullRequest = strategy === "pr" ||
        (push.branch === push.defaultBranch &&
            (strategy === "pr_default" || strategy === "pr_default_commit"));
    const createCommit = strategy === "commit" ||
        (push.branch === push.defaultBranch && strategy === "commit_default") ||
        (push.branch !== push.defaultBranch &&
            strategy === "pr_default_commit");
    if (!createPullRequest && !createCommit) {
        return status
            .success(`Skipping because of selected push strategy (${strategy}), ` +
            `branch (${push.branch}), and default branch (${push.defaultBranch})`)
            .hidden();
    }
    console_1.debug(`Attempting to persist changes with: ${JSON.stringify({
        strategy,
        push,
        pullRequest,
        commit,
    ...
    if (createPullRequest) {
        changedFiles.push(...(await git.editCommit(project, commit.editors, commit.message, author)));
        return await ensurePullRequest(ctx, project, Object.assign(Object.assign({}, pullRequest), { branch, changedFiles }), push);
    }
    else {
        ...
        return status.success(`Pushed changes to [${slug}/${branch}](${repoUrl})`);
    }
}
 ...
 * @param push push that triggered this activity
 */
async function ensurePullRequest(ctx, project, pullRequest, push) {
    var _a, _b, _c, _d, _e;
    const gh = operation_1.api(project.id);
    const files = uniq(pullRequest.changedFiles).sort();
    const hashes = [];
    for (const file of files) {
        const content = (await fs.readFile(project.path(file))).toString("base64");
        hashes.push({ path: file, hash: util_1.hash(content) });
    }
    const diffHash = util_1.hash(hashes);
    const slug = `${project.id.owner}/${project.id.repo}`;
    const repoUrl = `https://github.com/${slug}`;
    const body = (text) => check_1.truncateText(`${text.trim()}
...
${files
        .map(f => `-   [\`${f}\`](https://github.com/${project.id.owner}/${project.id.repo}/blob/${pullRequest.branch}/${f})`)
        .join("\n")}
...
${operation_1.formatMarkers(ctx, `atomist-diff:${diffHash}`)}
`);
    ...
    if (!newPr) {
        ...
        if ((diffMatch === null || diffMatch === void 0 ? void 0 : diffMatch[1]) === diffHash) {
            pushRequired = false;
        }
    }
    if (pushRequired) {
    ...
    if (((_a = pullRequest.labels) === null || _a === void 0 ? void 0 : _a.length) > 0) {
        // Remove prefixed labels that are getting set explicitly
        const prefixes = uniq(pullRequest.labels
            .filter(p => p.includes(":"))
            .map(p => p.split(":")[0] + ":"));
    ...
    if (!((_b = pullRequest.labels) === null || _b === void 0 ? void 0 : _b.includes("auto-merge:on-check-success")) &&
        (pullRequest.assignReviewer !== false ||
            ((_c = pullRequest.reviewers) === null || _c === void 0 ? void 0 : _c.length) > 0)) {
        const reviewers = [...(pullRequest.reviewers || [])];
        if (pullRequest.assignReviewer !== false &&
            ((_d = pr.user) === null || _d === void 0 ? void 0 : _d.login) !== push.author.login) {
    ...
    if (pullRequest.update) {
        ...
        if (((_e = update.reviewers) === null || _e === void 0 ? void 0 : _e.length) > 0) {
            await gh.pulls.requestReviewers({
                owner: project.id.owner,
                repo: project.id.repo,
                pull_number: pr.number,
    ...
    if (pushRequired) {
        return status.success(`Pushed changes to [${slug}/${pullRequest.branch}](${repoUrl}) and ${newPr ? "raised" : "updated"} [#${pr.number}](${pr.html_url})`);
    }
    else {
        return status.success(`No changes pushed to ` +
 ...
 */
async function closePullRequests(ctx, project, base, head, comment) {
    const gh = operation_1.api(project.id);
    const openPrs = await gh.paginate(gh.pulls.list, {
        owner: project.id.owner,
        repo: project.id.repo,
