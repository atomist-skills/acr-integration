import { Action as SlackAction, Attachment, SlackMessage } from "@atomist/slack-messages";
import { GraphQLClient } from "./graphql";
import { CommandContext, EventContext, HandlerStatus, WebhookContext } from "./handler/handler";
import { CommandIncoming, EventIncoming, Skill, Source, SubscriptionIncoming, WebhookIncoming } from "./payload";
export interface Destinations {
    users?: string | string[];
    channels?: string | string[];
}
export declare enum AttachmentTarget {
    Push = "push",
    Commit = "commit"
}
export interface MessageClient {
    ...
    attach?(attachment: Attachment, target: AttachmentTarget, identifier: string, name: string, ts: number): Promise<void>;
}
...
export declare abstract class MessageClientSupport implements MessageClient, CommandMessageClient {
    ...
    abstract attach(attachment: Attachment, target: AttachmentTarget, identifier: string, name: string, ts: number): Promise<void>;
...
export declare abstract class AbstractMessageClient extends MessageClientSupport {
    protected readonly request: CommandIncoming | EventIncoming | WebhookIncoming | SubscriptionIncoming;
    ...
    constructor(request: CommandIncoming | EventIncoming | WebhookIncoming | SubscriptionIncoming, correlationId: string, team: {
        id: string;
        name?: string;
...
export declare function mapBlockActions(msg: SlackMessage): Action[];
...
declare abstract class AbstractPubSubMessageClient extends AbstractMessageClient {
    ...
    constructor(request: CommandIncoming | EventIncoming | WebhookIncoming | SubscriptionIncoming, correlationId: string, team: {
        id: string;
        name?: string;
    }, source: Source, workspaceId: string, graphClient: GraphQLClient);
...
export declare class PubSubEventMessageClient extends AbstractPubSubMessageClient implements StatusPublisher {
    ...
    protected readonly correlationId: string;
    constructor(request: EventIncoming | SubscriptionIncoming, graphClient: GraphQLClient, teamId: string, teamName: string, operationName: string, correlationId: string);
    protected doSend(msg: string | SlackMessage, destinations: Destinations, options?: MessageOptions): Promise<any>;
    publish(status: HandlerResponse["status"]): Promise<void>;
}
export declare class PubSubWebhookMessageClient extends AbstractPubSubMessageClient implements StatusPublisher {
    protected readonly request: WebhookIncoming;
    protected readonly graphClient: GraphQLClient;
    constructor(request: WebhookIncoming, graphClient: GraphQLClient);
    protected doSend(msg: string | SlackMessage, destinations: Destinations, options?: MessageOptions): Promise<any>;
    publish(status: HandlerResponse["status"]): Promise<void>;
}
export declare function prepareStatus(status: HandlerStatus | Error, context: EventContext | CommandContext | WebhookContext): HandlerResponse["status"];
export {};
